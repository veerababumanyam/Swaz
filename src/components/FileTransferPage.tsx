...
import { saveScheduledJob, getScheduledJob, clearScheduledJob, ScheduledJobFromDB } from '../utils/scheduledTransferDB';
import TransferHistory from '../../components/TransferHistory';
import ErrorNotificationModal from '../../components/ErrorNotificationModal';
import FileReselectionPrompt from './FileReselectionPrompt';
...
const FileTransferPage: React.FC = () => {
    console.log('🚀 FileTransferPage component initialized');
    ...
    const [jobToReselect, setJobToReselect] = useState<ScheduledJobFromDB | null>(null);
    ...
    // FIX: Moved all helper function definitions before useEffect hooks to resolve hoisting issues.
    ...
    const sendMessage = (type: string, payload: any) => {
        ...
        console.log('📤 Current roomId:', roomId || joinRoomId);
        
        if (ws.current?.readyState === WebSocket.OPEN) {
            const messagePayload = { ...payload, roomId: roomId || joinRoomId };
            console.log('📤 Message payload:', messagePayload);
            const message = JSON.stringify({ type, payload: messagePayload });
            console.log('📤 Full message JSON:', message);
            ...
            console.error('📤 Cannot send message - WebSocket not open. State:', ws.current?.readyState);
    ...
    const handleSignalingMessage = async (message: MessageEvent) => {
        console.log('📨 Raw signaling message received:', message.data);
        try {
            const data = JSON.parse(message.data);
            console.log('📨 Parsed signaling message:', data);
            
            if (!webRTCManager.current || !encryptionPipeline.current || !fileManager.current) {
                console.error("❌ Modules not initialized! webRTCManager:", !!webRTCManager.current, "encryptionPipeline:", !!encryptionPipeline.current, "fileManager:", !!fileManager.current);
                return;
            }
            ...
            console.error('❌ Failed to parse signaling message:', error, 'Raw data:', message.data);
        }
    };

    const connectWebSocket = (onOpenCallback: () => void) => {
        console.log('🔌 Attempting to connect to WebSocket:', SIGNALING_SERVER_URL);
        console.log('🔌 Current window location:', window.location.href);
        console.log('🔌 WebSocket protocol:', window.location.protocol);
        if (ws.current && ws.current.readyState < 2) {
             console.log('🔌 WebSocket already exists with state:', ws.current.readyState);
        ...
        ws.current.onopen = () => {
            ...
            console.log('🔌 WebSocket readyState after open:', ws.current?.readyState);
            // Add a small delay to ensure connection is fully established
            setTimeout(() => {
                console.log('🔌 Calling onOpenCallback after delay');
                onOpenCallback();
            }, 100);
        };
        ...
        ws.current.onclose = (event) => {
            ...
            if (peerConnectedRef.current) setStatusInternal({ type: 'error', message: 'Signaling server disconnected.' });
        };
    };

    const initializeModules = () => {
        console.log('🔧 initializeModules called');
        if (webRTCManager.current) {
            console.log('🔧 Modules already initialized, returning');
            return;
        }
        
        const onFileSentOrReceived = async (file: {name: string; size: number; type: string;}, status: 'Sent' | 'Received') => {
            const duration = transferStartTime ? (Date.now() - transferStartTime) / 1000 : 0;
        ...
        console.log('🔧 initializeModules completed - all modules created');
    };

    const handleStartSending = (selectedFiles: File[]) => {
        console.log('🚀 handleStartSending called with files:', selectedFiles.length);
        setFilesToSend(selectedFiles);
        setTransferStartTime(null);
        setAverageSpeed(0);
        setSpeedDataPoints([]);
        setProgress({});
        console.log('🚀 About to call initializeModules');
        initializeModules();
    ...
    const handleFilesReselected = (files: File[], job: ScheduledJobFromDB) => {
        ...
        initializeModules();
        connectWebSocket(() => sendMessage('join-room', { roomId: job.roomId }));
        setTransferState('scheduled');
        
        const timeRemaining = job.scheduledTime - Date.now();
        if (timeRemaining > 0) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            progressHistory.current = progressHistory.current.filter(p => now - p.time < 5000);
            ...
            <div className="flex flex-col items-center text-center p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg h-full">
                ...
                <div className="w-full">
                    <input 
                        ...
                        placeholder="Enter Room ID (e.g., 06f0be8e-32d0-4b15-b)"
    ...
    const renderContent = () => {
        if (jobToReselect) {
            return <FileReselectionPrompt
                job={jobToReselect}
                onFilesReady={(files) => handleFilesReselected(files, jobToReselect)}
                onCancel={() => {
                    clearScheduledJob();
                    setJobToReselect(null);
                }}
        ...
        if (view === 'host') {
            return <SenderView
                ...
                onShareRoomId={handleShareRoomId}
                files={filesToSend}
                progress={progress}
